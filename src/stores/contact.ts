...
export const useContactStore = defineStore('contact', () => {
    ...
    const loadContactInfo = async () => {
        try {
            ...
            // const response = await useApi().get<ContactInfoResponse>('/api/v1/settings/contact')
            const response = await useApi().get('/api/v1/settings/contact')
    ...
    const validateForm = (form: ContactForm): FormFieldError[] => {
        ...
        if (validationRules.name.required && (!form.name || form.name.trim().length === 0)) {
            ...
            errors.push({
                field: 'name',
                message: `Họ và tên phải có ít nhất ${validationRules.name.minLength} ký tự`,
            })
        } else if (form.name && form.name.length > validationRules.name.maxLength) {
            errors.push({
                field: 'name',
                message: `Họ và tên không được vượt quá ${validationRules.name.maxLength} ký tự`,
            })
        }
        ...
        if (validationRules.email.required && (!form.email || form.email.trim().length === 0)) {
            ...
            errors.push({
                field: 'email',
                message: `Email không được vượt quá ${validationRules.email.maxLength} ký tự`,
            })
        }
        ...
        if (form.phone && !validationRules.phone.pattern.test(form.phone)) {
            ...
            errors.push({
                field: 'phone',
                message: `Số điện thoại không được vượt quá ${validationRules.phone.maxLength} ký tự`,
            })
        }
        ...
        if (
            ...
            (!form.subject || form.subject.trim().length === 0)
        ) {
            ...
            errors.push({
                field: 'subject',
                message: `Tiêu đề không được vượt quá ${validationRules.subject.maxLength} ký tự`,
            })
        }
        ...
        if (
            ...
            (!form.message || form.message.trim().length === 0)
        ) {
            ...
            errors.push({
                field: 'message',
                message: `Nội dung tin nhắn phải có ít nhất ${validationRules.message.minLength} ký tự`,
            })
        } else if (form.message && form.message.length > validationRules.message.maxLength) {
            errors.push({
                field: 'message',
                message: `Nội dung tin nhắn không được vượt quá ${validationRules.message.maxLength} ký tự`,
            })
        }
    ...
    const submitContactForm = async (form: ContactForm): Promise<boolean> => {
        try {
            ...
            formState.value.successMessage =
                'Cảm ơn bạn đã liên hệ với chúng tôi! Chúng tôi sẽ phản hồi lại sớm nhất có thể.'
